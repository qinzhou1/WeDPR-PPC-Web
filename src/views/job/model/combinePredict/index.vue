        ...
        >
          <template slot="keyFilter">
            <el-form inline class="demo-form-inline" size="medium">
              ...
              <el-form-item :label="$t('job.predictType')">
                <el-select
                  v-model="controllerData.keyFilterData.jobAlgorithmSubtype"
                  clearable
                  :placeholder="$t('job.selectPredictType')"
                >
...
export default {
  ...
  data() {
    return {
      ...
      controllerData: {
        ...
        createButtonName: this.$t('job.new.createPredict'),
        startDate: 0,
        endDate: 0,
        keyFilterData: {
          jobAlgorithmSubtype: '',
  ...
  methods: {
    ...
    onCreate() {
      this.$router.push({ path: 'combinePredictCreate' })
    },
    fetchAgencyList() {
    ...
    fetchJobs(params) {
      ...
      // 预测对应的任务类型
      params.jobAlgorithmTypeList = 'PPC_PREDICT'
    ...
    showJobDetailByColumn(jobId) {
      this.$router.push({ path: 'combinePredictDetail', query: { jobId }})
    },
    showJobResultByColumn(jobId, jobAlgorithmType) {
      this.$router.push({ path: 'combinePredictResult', query: { jobId, jobAlgorithmType }})
    },
    onClickRerun(job) {
      ...
      this.$confirm(confirmMsg, this.$t('common.tip'), {
        ...
        rerunJob(job.jobId).then(response => {
          if (handleOptResponseMsgBox(this, response, true, this.$t('job.index.runTip'))) {
            this.showJobDetailByColumn(job.jobId, job.jobAlgorithmType, 'WAITING', job.jobCreator, job.jobResultReceiverList)
          }
        })
      })
    },
