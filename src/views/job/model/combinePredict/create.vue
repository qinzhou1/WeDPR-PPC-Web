...
export default {
  ...
  methods: {
    ...
    getAlgorithmsList() {
      ...
      params = { ...params, pageOffset: (page_offset - 1) * page_size, pageSize: page_size, algorithmType: 'PPC_PREDICT' }
    ...
    onCreate() {
      ...
      // 生成名称和描述
      this.jobBasicData.jobTitle = 'predict-' + this.selectedDataset.map(v => v.datasetTitle).join('-')
      ...
      this.$msgbox(
        ...
        createJob(createJobParams).then(response => {
          if (handleOptResponseMsgBox(this, response, true)) {
            const { data = '' } = response
            this.$router.push({ path: 'combinePredictDetail', query: { jobId: data }})
          }
        })
      }).catch(error => {
        if (error === 'cancel') {
